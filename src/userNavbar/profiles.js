...
const [profileUser, setProfileUser] = useState(null); // user being viewed
  ...
  const fetchProfileUser = async () => {
    try {
      const response = await fetch(`https://server-t48e.onrender.com/api/users/${id}`, {
        credentials: "include",
      });
  ...
  const fetchPosts = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const userRes = await fetch("https://server-t48e.onrender.com/api/users", { credentials: "include" });
  ...
  const handleMessageClick = async () => {
    ...
    try {
      const res = await fetch(`https://server-t48e.onrender.com/api/messages/${user.id}/${userToChat.id}`);
      const data = await res.json();
