  ...
  const handleSubmit = async (e) => {
    ...
    if (!validateAll()) return;
      try {
        ...
        const response =  await fetch("https://server-t48e.onrender.com/api/submit",{
          method: "POST",
          headers: {
    ...
    try {
      const response = await fetch("https://server-t48e.onrender.com/api/user", { credentials: "include" });
      if (response.ok) {
