...
const onSubmit = async (e) => {
  ...
  try {
    const res = await fetch("https://server-t48e.onrender.com/api/submitsurvey", {
      method: "POST",
...
useEffect(() => {
  const fetchLaterCount = async () => {
    try {
      const res = await fetch("https://server-t48e.onrender.com/api/getlatercount", { credentials: "include" });
...
const handleFillOutLater = async () => {
  try {
    ...
    const res = await fetch("https://server-t48e.onrender.com/api/incrementlater", {
      method: "POST",
      credentials: "include",
    });
