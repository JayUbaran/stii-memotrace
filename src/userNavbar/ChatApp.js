...
const ChatApp = () => {
 ...
 useEffect(() => {
  ...
  const loadConversations = () => {
    ...
    fetch(`https://server-t48e.onrender.com/api/messages/${currentUser.id}/conversations`)
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    ...
    fetch(`https://server-t48e.onrender.com/api/messages/${currentUser.id}/${selectedUser.partner_id}`)
      .then((res) => res.json())
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch("https://server-t48e.onrender.com/api/messages", {
        method: "POST",
  ...
  const handleHideConversation = async (partnerId) => {
    try {
      await fetch(`https://server-t48e.onrender.com/api/messages/${currentUser.id}/${partnerId}/hide`, {
        method: "PATCH",
      });
      loadConversations();
      setActiveMenu(null);
  ...
  const handleDeleteConversation = async (partnerId) => {
    try {
      await fetch(`https://server-t48e.onrender.com/api/messages/${currentUser.id}/${partnerId}/delete`, {
        method: "DELETE",
      });
      loadConversations();
